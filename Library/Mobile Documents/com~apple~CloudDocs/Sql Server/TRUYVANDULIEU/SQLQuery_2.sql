...
-- truy van
Select * FROM dbo.BOMON -- lay het toan bo bang
 -- lay MABM, TENBM TU TABLE BOMON
SELECT MABM, TENBM FROM dbo.BOMON
SELECT *FROM dbo.GIAOVIEN 
-- DOI TENCOT HIEN THI
SELECT MABM AS 'MA BO MON', TENBM AS 'TEN BO MON'
FROM dbo.BOMON 
-- TRUY VAN DU LIEU CAR HAI BANG
SELECT gv.MAGV, gv.HOTEN, bm.TENBM 
FROM dbo.GIAOVIEN as gv, dbo.BOMON AS bm
-- bai tap
-- 1 truy xuat thong tin cua bang tham gia de tai
SELECT * FROM GIAOVIEN
-- 2 lay ra tên khoa va ma khoA tuong ung
select MAKHOA, TENKHOA FROM dbo.KHOA
-- 3 LAY RA MA GV, TEN GV, HO TEN NGUOI THAN TƯƠNG UNG
SELECT GV.MAGV, GV.HOTEN, NT.TEN 
FROM dbo.GIAOVIEN AS GV, dbo.NGUOITHAN AS NT
WHERE GV.MAGV = NT.MAGV
-- 4 lay ra magv, tengv, ten khoa gv do lam viec
	SELECT GV.MAGV, GV.HOTEN, khoa.TENKHOA
	FROM dbo.GIAOVIEN AS GV, dbo.KHOA AS khoa, dbo.BOMON AS BM
	WHERE GV.MABM = BM.MABM AND BM.MAKHOA = khoa.MAKHOA

-- LUONG HV > 2000
SELECT GV.HOTEN, GV.LUONG FROM GIAOVIEN AS GV
WHERE GV.LUONG > 2000
-- GV LÀ NỮ
SELECT * FROM DBO.GIAOVIEN
...
-- Lấy ra họ ten gv, năm sinh,  và tuổi của giáo viên nhỏ hơn 45 tuổi
SELECT HOTEN, NGSINH, YEAR(GETDATE()) - YEAR(NGSINH) AS 'TUỔI' 
FROM dbo.GIAOVIEN
WHERE YEAR(GETDATE()) - YEAR(NGSINH) < 45;

SELECT *FROM dbo.BOMON
-- Ly ra gv la truong bo mon
SELECT GV.*FROM dbo.GIAOVIEN AS GV, dbo.BOMON AS BM
WHERE GV.MAGV = BM.TRUONGBM
-- lay ra so nguoi than cua  ma gv 007
SELECT COUNT(*) AS N'SO NGUOI THAN CUA GIAO VIEN' 

FROM DBO.GIAOVIEN AS GV, dbo.NGUOITHAN 
WHERE GV.MAGV =  007 AND GV.MAGV = NGUOITHAN.MAGV

----------XUAT RA THONG TIN GIAO VIEN VA GIAO VIEN QUAN LI CHU NHIEM DO
SELECT GV1.HOTEN, GV2.HOTEN FROM DBO.GIAOVIEN AS GV1, dbo.GIAOVIEN AS GV2
WHERE GV1.GVQLCM = GV2.MAGV 
--XUAT RA SO LUONG GV KHOA CNTT
SELECT COUNT(*) FROM DBO.GIAOVIEN AS GV, DBO.BOMON AS BM, DBO.KHOA AS KHOA
WHERE GV.MABM = BM.MABM
AND   BM.MAKHOA = KHOA.MAKHOA
AND   KHOA.MAKHOA = 'CNTT'

--xuat ra  thong tin giao vien va de tai nguoi do tham gia khi ket qua la daat
SELECT *FROM dbo.GIAOVIEN as GV, dbo.THAMGIADT AS TG 
WHERE GV. MAGV = TG.MAGV
AND TG.KETQUA = N'Đạt'
-- xuat ra thong tin giao vien va co chu n cuoi
SELECT * FROM dbo.GIAOVIEN
WHERE HOTEN LIKE '%n'
